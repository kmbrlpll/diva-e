...
function TemperatureIndicator (props) { 
  let styleText =  {fontSize: 8 ,color: assignThermometerColor(props.thermometerData.state)};
  ...
  return (
      ...
      top = {(props.thermometerData.y / props.imageDimensions.y * 100).toString() + "%"}
     	style ={styles.thermo}  
      borderColor = { assignThermometerColor(props.thermometerData.state)}
      >
        <Text 
        style={styleText}
        >{props.thermometerData.state}°
        </Text>
      </View>
	) 
} 
